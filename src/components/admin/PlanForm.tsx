...
export function PlanForm({
  ...
  function removeFeature(id: string) {
    set("features", values.features.filter((f) => f.id !== id));
  }
  ...
  function submit(active: boolean) {
    ...
    if (values.features.some((f) => !f.text.trim())) return toast.error("Remove or fill in empty feature lines");
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <div className="space-y-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <Field label="Plan key">
              <div className="relative">
                ...
                >
                  {PLAN_KEYS.map((k) => <option key={k.value} value={k.value}>{k.value}</option>)}
                </select>
            ...
            <Field label="Display name">
              <input value={values.name} onChange={(e) => set("name", e.target.value)} placeholder="e.g. 3 Months Access" className="h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </Field>
            <Field label="Price (GHS)">
              <input type="number" min={0} value={values.price} onChange={(e) => set("price", Number(e.target.value))} className="h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </Field>
            <Field label="Duration (days)">
              <input type="number" min={1} value={values.durationDays} onChange={(e) => set("durationDays", Number(e.target.value))} className="h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </Field>
            <Field label="Badge label (optional)">
              <input value={values.badgeLabel} onChange={(e) => set("badgeLabel", e.target.value)} placeholder="Most Popular" className="h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </Field>
            <Field label="Sort order">
              <input type="number" value={values.sortOrder} onChange={(e) => set("sortOrder", Number(e.target.value))} className="h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono text-sm text-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </Field>
          </div>
          <label className="mt-4 flex items-center gap-2 text-sm text-foreground">
            <input type="checkbox" checked={values.active} onChange={(e) => set("active", e.target.checked)} className="h-4 w-4 accent-[var(--color-primary)]" />
        ...
        <section className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-4 space-y-2">
            {values.features.length === 0 && (
              <p className="rounded-lg border border-dashed border-border py-6 text-center text-xs text-muted-foreground">No features yet — add your first line.</p>
            )}
            {values.features.map((f, i) => (
              ...
              >
                <span className="flex cursor-grab items-center text-muted-foreground active:cursor-grabbing" title="Drag to reorder">
                  <GripVertical className="h-4 w-4" />
                </span>
                <button
                  type="button"
                  onClick={() => updateFeature(f.id, { included: !f.included })}
                  title={f.included ? "Included — click to mark excluded" : "Excluded — click to mark included"}
                ...
                />
                <button type="button" onClick={() => removeFeature(f.id)} className="rounded-md p-1.5 text-muted-foreground hover:bg-error/10 hover:text-error" aria-label="Remove feature">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link to="/admin/settings/plans" className="inline-flex h-11 items-center rounded-lg border border-border bg-surface px-5 text-sm font-semibold text-foreground hover:bg-surface-alt">Cancel</Link>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => submit(true)} className="inline-flex h-11 items-center gap-2 rounded-lg bg-gradient-to-r from-primary to-accent px-6 text-sm font-bold text-white shadow-md hover:opacity-90">
...
function PlanPreview({ values }: { values: PlanFormValues }) {
  return (
    <div className={`relative overflow-hidden rounded-2xl border bg-surface p-6 shadow-[var(--shadow-card)] ${values.badgeLabel ? "border-primary/40 ring-1 ring-primary/20" : "border-border"}`}>
      {values.badgeLabel && (
      ...
      <div className="mt-3 flex items-end gap-1">
        <span className="text-3xl font-extrabold tracking-tight text-foreground">GHS {values.price.toLocaleString()}</span>
        <span className="pb-1 text-xs text-muted-foreground">{durationLabel(values.planKey)}</span>
      </div>
      <ul className="mt-5 space-y-2.5">
        {values.features.filter((f) => f.text.trim()).map((f) => (
          <li key={f.id} className="flex items-start gap-2 text-sm">
            <span className={`mt-0.5 flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full ${f.included ? "bg-success/15 text-success" : "bg-error/10 text-error"}`}>
      ...
      <button type="button" disabled className="mt-6 h-11 w-full rounded-lg bg-gradient-to-r from-primary to-accent text-sm font-bold text-white opacity-90">Subscribe</button>
      {!values.active && <p className="mt-2 text-center text-[11px] font-semibold text-warning">Hidden — not shown to customers</p>}
    </div>
  );
}
