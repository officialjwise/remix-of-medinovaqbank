...
function Shell({
  ...
  return (
    <div
      className="fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-foreground/50 p-4 pt-16 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
      ...
      >
        <header className="flex items-center justify-between border-b border-border px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-muted-foreground hover:bg-surface-alt"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
...
const inputCls =
  "h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20";
...
export function EditUserModal({
  ...
  user: BaseUser;
  onClose: () => void;
  onSave: (patch: Partial<BaseUser>) => void;
}) {
...
export function ComposeEmailModal({ user, onClose }: { user: BaseUser; onClose: () => void }) {
  ...
  function applyTemplate(key: string) {
    ...
    setSubject(
      t.subject
        .replace(/\{\{userName\}\}|\{\{name\}\}/g, user.name)
    ...
    >
      <Labeled label="To">
        <input
          readOnly
          value={user.email}
          className={`${inputCls} bg-surface-alt/60 text-muted-foreground`}
        />
      </Labeled>
...
export function FlagAccountModal({
  user,
  onClose,
  onFlag,
}: {
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground">
        Flagging adds a review marker to <strong className="text-foreground">{user.name}</strong>'s
        account.
      </p>
      <div className="flex flex-wrap gap-2">
        {presets.map((p) => (
          <button
            ...
            type="button"
            onClick={() => setReason(p)}
            className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${reason === p ? "border-warning bg-warning/10 text-warning" : "border-border text-muted-foreground hover:text-foreground"}`}
          >
            {p}
          </button>
        ))}
