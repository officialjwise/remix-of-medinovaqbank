        ...
        <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              The master list of platform capabilities you can attach to any plan or the free trial.{" "}
              ...
              <tr key={f.key} className="hover:bg-surface-alt/30">
                <td className="px-5 py-3.5">
                  <div className="flex items-start gap-3">
                    <span
                      className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg ${f.type === "boolean" ? "bg-accent/10 text-accent" : "bg-primary/10 text-primary"}`}
                    >
                      {f.type === "boolean" ? (
                ...
                <td className="px-5 py-3.5">
                  <span
                    className={`inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${f.type === "boolean" ? "bg-accent/10 text-accent" : "bg-primary/10 text-primary"}`}
                  >
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex h-8 items-center justify-center rounded-md border border-error/30 bg-error/5 px-2.5 text-error hover:bg-error/10"
                    >
        ...
        <div className="space-y-4 p-5">
          ...
          <Labeled label="Key">
            <input
              ...
              disabled={!!initial}
              className="h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono text-sm disabled:opacity-60 focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20"
            />
          </Labeled>
            ...
            <div className="flex gap-2">
              {(["boolean", "limit"] as FeatureType[]).map((t) => (
                <button
                  ...
                  type="button"
                  onClick={() => setType(t)}
                  className={`flex-1 rounded-lg border px-3 py-2 text-sm font-semibold ${type === t ? "border-accent bg-accent/10 text-accent" : "border-border bg-surface text-muted-foreground hover:text-foreground"}`}
                >
          ...
          <button
            ...
            onClick={() =>
              onSave({
                ...
                type,
                defaultLimit: type === "limit" ? defaultLimit : undefined,
              })
            }
