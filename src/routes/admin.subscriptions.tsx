...
const seed: SubRow[] = [
  { id: "p-1", user: "Akua Mensah", email: "akua@example.com", plan: "12 Months", amount: 799, start: "2025-12-01", end: "2026-12-01", status: "Active", ref: "PSK-9F2A1B" },
  { id: "p-2", user: "Kwame Boateng", email: "kwame@example.com", plan: "Monthly", amount: 129, start: "2026-04-22", end: "2026-05-22", status: "Active", ref: "PSK-7C8E33" },
  { id: "p-3", user: "Adjoa Owusu", email: "adjoa@example.com", plan: "3 Months", amount: 299, start: "2026-03-15", end: "2026-06-15", status: "Active", ref: "PSK-AA901D" },
  { id: "p-4", user: "Kofi Adu", email: "kofi@example.com", plan: "6 Months", amount: 499, start: "2025-11-04", end: "2026-05-04", status: "Refunded", ref: "PSK-2D77E1" },
  { id: "p-5", user: "Esi Quaye", email: "esi@example.com", plan: "Monthly", amount: 129, start: "2026-04-09", end: "2026-05-09", status: "Failed", ref: "PSK-1F4422" },
  { id: "p-6", user: "Nana Appiah", email: "nana@example.com", plan: "12 Months", amount: 799, start: "2025-07-15", end: "2026-07-15", status: "Active", ref: "PSK-3B11C2" },
  { id: "p-7", user: "Yaw Asante", email: "yaw@example.com", plan: "3 Months", amount: 299, start: "2026-02-10", end: "2026-05-10", status: "Cancelled", ref: "PSK-8E55F0" },
  { id: "p-8", user: "Efua Asare", email: "efua@example.com", plan: "Monthly", amount: 129, start: "2026-04-01", end: "2026-05-01", status: "Active", ref: "PSK-44A0B7" },
];
...
function AdminSubscriptions() {
  ...
  const thisMonth = filtered
    .filter((r) => r.status === "Active" && new Date(r.start).getMonth() === now.getMonth() && new Date(r.start).getFullYear() === now.getFullYear())
    .reduce((a, r) => a + r.amount, 0);
  const lastMonthDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const lastMonth = filtered
    .filter((r) => r.status === "Active" && new Date(r.start).getMonth() === lastMonthDate.getMonth() && new Date(r.start).getFullYear() === lastMonthDate.getFullYear())
    .reduce((a, r) => a + r.amount, 0);
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{filtered.length} of {rows.length} transactions</p>
        </div>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-end gap-3 rounded-xl border border-border bg-surface p-4">
        <FilterSelect label="Plan" value={plan} onChange={(v) => setPlan(v as typeof plan)} options={PLANS as readonly string[]} />
        ...
        {(plan !== "All" || status !== "All" || from || to) && (
          <button
            onClick={() => { setPlan("All"); setStatus("All"); setFrom(""); setTo(""); }}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((s) => (
            <SubscriptionRow
              key={s.id}
              row={s}
              onUpdate={(updated) => setRows((rs) => rs.map((r) => r.id === updated.id ? updated : r))}
            />
          ))
        )}
      </div>
...
function SubscriptionRow({ row, onUpdate }: { row: SubRow; onUpdate: (r: SubRow) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-2 border-b border-border px-5 py-3 last:border-b-0 md:grid-cols-[1.5fr_120px_100px_110px_110px_110px_140px_60px] md:items-center md:gap-4">
      ...
      <span className="text-right font-mono text-sm font-bold tabular-nums text-foreground">GHS {row.amount}</span>
        ...
        {open && (
          <>
            ...
            <div className="absolute right-0 z-40 mt-1 w-52 overflow-hidden rounded-lg border border-border bg-surface shadow-xl">
              <MenuItem icon={<Eye className="h-4 w-4" />} label="View Details" onClick={() => { setDetails(true); setOpen(false); }} />
              <MenuItem icon={<Calendar className="h-4 w-4" />} label="Override End Date" onClick={() => { setOverride(true); setOpen(false); }} />
              {row.status === "Active" && (
                <MenuItem icon={<Ban className="h-4 w-4" />} label="Cancel Subscription" destructive onClick={() => { setCancel(true); setOpen(false); }} />
              )}
            </div>
          </>
        )}
      ...
      {override && (
        <OverrideEndDateModal
          ...
          onSave={(end) => { onUpdate({ ...row, end }); toast.success("End date updated"); setOverride(false); }}
        />
      )}
      <ConfirmDialog
        open={cancel}
        title="Cancel this subscription?"
        description={<>This will end <span className="font-semibold text-foreground">{row.user}</span>'s plan <span className="font-semibold text-foreground">{row.plan}</span> immediately. They will lose access at the end of the current billing period.</>}
        confirmLabel="Cancel Subscription"
        cancelLabel="Keep Active"
        variant="destructive"
        typedConfirmation="CANCEL"
        onConfirm={() => { onUpdate({ ...row, status: "Cancelled" }); toast.success("Subscription cancelled"); setCancel(false); }}
...
function StatusPill({ status }: { status: SubRow["status"] }) {
  const cls =
    ...
    status === "Cancelled" ? "bg-warning-light text-warning" :
    status === "Refunded" ? "bg-warning-light text-warning" :
    status === "Expired" ? "bg-surface-alt text-muted-foreground" :
...
function RevenueCard({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div className={`rounded-xl border p-5 ${accent ? "border-accent/40 bg-accent/5" : "border-border bg-surface"}`}>
...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: readonly string[] }) {
  return (
    <label className="flex flex-col text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
      {label}
      <select value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 h-9 rounded-lg border border-border bg-surface px-2.5 text-sm font-medium normal-case tracking-normal text-foreground focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20">
...
function FilterDate({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return (
    <label className="flex flex-col text-[10px] font-bold uppercase tracking-wide text-muted-foreground">
      {label}
      <input type="date" value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 h-9 rounded-lg border border-border bg-surface px-2.5 text-sm font-medium normal-case tracking-normal text-foreground" />
    </label>
  );
}
...
function OverrideEndDateModal({ row, onClose, onSave }: { row: SubRow; onClose: () => void; onSave: (end: string) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 pt-16" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-surface shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3 p-5">
          <p className="text-sm text-muted-foreground">{row.user} · {row.plan}</p>
          <label className="block">
            <span className="mb-1 block text-xs font-bold uppercase tracking-wide text-muted-foreground">New end date</span>
            <input type="date" value={end} onChange={(e) => setEnd(e.target.value)} className="h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm" />
          </label>
        </div>
        <footer className="flex justify-end gap-2 border-t border-border px-5 py-3">
          ...
          <button onClick={() => onSave(end)} className="inline-flex h-10 items-center rounded-lg bg-accent px-4 text-sm font-semibold text-accent-foreground hover:bg-accent/90">Save</button>
        </footer>
      </div>
    </div>
  );
