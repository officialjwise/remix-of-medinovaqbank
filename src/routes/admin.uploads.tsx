...
export const Route = createFileRoute("/admin/uploads")({
  head: () => ({ meta: [{ title: "Admin · Bulk Uploads — Medinovaqbank" }, { name: "robots", content: "noindex" }] }),
  component: AdminUploads,
});

const COLUMNS = ["question number", "question stem", "options (A-D)", "right option", "difficulty (optional)", "topic (optional)"];
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Import questions into any bank from a CSV or XLSX file.</p>
      </div>
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-card)]">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Choose the target bank, then drag and drop your file.</p>
          </div>
        </div>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex-1">
            <span className="mb-1.5 block text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Target bank</span>
            <select
              value={bankId}
            ...
            >
              ...
              {questionBanks.map((b) => (
                <option key={b.id} value={b.id}>{b.name} · {b.questionCount.toLocaleString()} questions</option>
              ))}
            </select>
          </label>
          <button
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-card)]">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Start from a correctly formatted file.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <button onClick={() => downloadTemplate("csv")} className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-surface px-4 text-sm font-semibold text-foreground hover:bg-surface-alt">
              <Download className="h-4 w-4 text-primary" /> CSV Template
            </button>
            <button onClick={() => downloadTemplate("xlsx")} className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-surface px-4 text-sm font-semibold text-foreground hover:bg-surface-alt">
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-[var(--shadow-card)]">
          ...
          <p className="mt-2 break-words font-mono text-xs leading-relaxed text-muted-foreground">{COLUMNS.join("  │  ")}</p>
          <p className="mt-3 rounded-lg bg-surface-alt/60 p-3 text-xs text-muted-foreground">
            Put <span className="font-semibold text-foreground">each option on its own line</span> inside the options cell, prefixed with its letter — e.g. <span className="font-mono text-foreground">A. First option</span>. The <span className="font-semibold text-foreground">right option</span> column is just the letter (A–E).
          </p>
        </div>
      </section>
      ...
      {/* Per-bank shortcuts */}
      <section>
        <h3 className="mb-3 text-sm font-bold uppercase tracking-wide text-muted-foreground">Upload into a specific bank</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {questionBanks.map((b) => (
            <button
              key={b.id}
              onClick={() => navigate({ to: "/admin/banks/$bankId/upload", params: { bankId: b.id } })}
            ...
            >
              <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg text-white" style={{ background: b.accentHex }}>
                <Library className="h-5 w-5" />
              </span>
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold text-foreground">{b.name}</span>
                <span className="block text-xs text-muted-foreground">{b.questionCount.toLocaleString()} questions</span>
              </span>
