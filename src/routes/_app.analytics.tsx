...
const SUBJECT_HEX = ["#0E7C7B", "#2BC97F", "#3B82F6", "#E89A1A", "#7C3AED", "#E11D48", "#15A89C", "#6366F1", "#F472B6"];
...
function buildTimeSeries(rangeKey: string, baseAvg: number) {
  ...
  for (let i = 0; i < points; i++) {
    ...
    const seconds = Math.round(95 - progress * 1.5 + (seededHash(`${rangeKey}|sec|${i}`) - 0.5) * 18);
...
function AnalyticsPage() {
  ...
  const bySubject = useMemo(() => {
    ...
    return data.bySubject.map((s) => ({
      name: s.name,
      pct: Math.max(35, Math.min(98, s.pct + Math.round((seededHash(`${bank}|${s.name}`) - 0.5) * 18))),
    }));
  }, [bank, data.bySubject]);
  ...
  const timeDelta = timeSeries.length > 1 ? timeSeries[0].seconds - timeSeries[timeSeries.length - 1].seconds : 0;
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <GradientKpiCard
          ...
          icon={CheckCircle2}
          gradient="emerald"
          trend={{ value: `${Math.round((data.totalCorrect / data.totalAnswered) * 100)}% accuracy`, up: true }}
        />
        <GradientKpiCard
          label="Best Subject"
            ...
            <div className="mt-4 h-[280px] w-full">
              <ResponsiveContainer>
                <AreaChart data={bell.map((p) => ({ ...p, left: p.x <= data.yourPct ? p.y : null, right: p.x >= data.yourPct ? p.y : null }))} margin={{ top: 24, right: 24, bottom: 0, left: 0 }}>
                  ...
                  <YAxis hide />
                  <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number) => (v == null ? "" : v.toFixed(3))} labelFormatter={(l) => `${l}%`} />
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <section className="rounded-2xl border border-border bg-surface p-6 lg:col-span-2">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Accuracy by subject{bank !== "All" ? " in this bank" : ""}.</p>
          <div className="mt-4 h-80 w-full">
            <ResponsiveContainer>
              <BarChart data={bySubject} layout="vertical" margin={{ left: 20, right: 24 }}>
                ...
                <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number) => `${v}%`} cursor={{ fill: "var(--color-border)", opacity: 0.3 }} />
            ...
            {topics.strengths.map((t) => (
              <li key={t.name} className="flex items-center justify-between gap-3">
                ...
                <div className="flex flex-1 items-center gap-3">
                  ...
                  <span className="w-10 text-right font-mono text-sm font-bold tabular-nums text-success">{t.pct}%</span>
                </div>
              </li>
            ))}
          </ul>
            ...
            {topics.weaknesses.map((t) => (
              <li key={t.name} className="flex items-center justify-between gap-3">
                ...
                <div className="flex flex-1 items-center gap-3">
                  ...
                  <span className="w-10 text-right font-mono text-sm font-bold tabular-nums text-error">{t.pct}%</span>
                </div>
              </li>
            ))}
          </ul>
        ...
        <div className="mt-4 h-64 w-full">
          <ResponsiveContainer>
            <AreaChart data={timeSeries} margin={{ top: 10, right: 24, bottom: 0, left: 0 }}>
              ...
              <Area type="monotone" dataKey="pct" stroke="#0E7C7B" strokeWidth={2.5} fill="url(#scoreTrend)" dot={{ r: 3, fill: "#0E7C7B" }} isAnimationActive={false} />
              <ReferenceLine y={headlineAvg} stroke="var(--color-muted-foreground)" strokeDasharray="3 3" label={{ value: `Avg ${headlineAvg}%`, fontSize: 10, fill: "var(--color-muted-foreground)", position: "right" }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <section className="rounded-2xl border border-border bg-surface p-6 lg:col-span-2">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Accuracy and volume across your question banks.</p>
          <div className="mt-4 h-72 w-full">
            <ResponsiveContainer>
              <BarChart data={perBank} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
                <CartesianGrid vertical={false} stroke="var(--color-border)" />
                <XAxis dataKey="short" stroke="var(--color-muted-foreground)" fontSize={11} interval={0} angle={-20} textAnchor="end" height={60} />
                <YAxis domain={[0, 100]} tickFormatter={(v) => `${v}%`} stroke="var(--color-muted-foreground)" fontSize={12} />
                <Tooltip
                  contentStyle={TOOLTIP_STYLE}
                  formatter={(v: number, _n, p) => [`${v}%`, (p?.payload as { name: string })?.name ?? "Accuracy"]}
            ...
            <div className="relative h-40 w-40">
              <ResponsiveContainer>
                <RadialBarChart
                  innerRadius="72%"
                  outerRadius="100%"
                  data={[{ name: "avg", value: Math.min(100, Math.round((avgTime / 120) * 100)), fill: "#0E7C7B" }]}
                  startAngle={90}
                  endAngle={-270}
                >
                  <PolarAngleAxis type="number" domain={[0, 100]} tick={false} />
                  <RadialBar dataKey="value" cornerRadius={10} background={{ fill: "var(--color-border)" }} isAnimationActive={false} />
                </RadialBarChart>
              </ResponsiveContainer>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-extrabold tabular-nums text-foreground">{avgTime}s</span>
                <span className="text-xs text-muted-foreground">avg / question</span>
              </div>
            </div>
          </div>
          <p className={`mt-4 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ${timeDelta >= 0 ? "bg-success-light text-success" : "bg-warning-light text-warning"}`}>
            {timeDelta >= 0 ? <ArrowDownRight className="h-3.5 w-3.5" /> : <ArrowUpRight className="h-3.5 w-3.5" />}
            {timeDelta >= 0 ? `${timeDelta}s faster` : `${Math.abs(timeDelta)}s slower`} than period start
          </p>
          <div className="mt-4 h-24 w-full">
            <ResponsiveContainer>
              <LineChart data={timeSeries} margin={{ top: 6, right: 8, bottom: 0, left: 0 }}>
                ...
                <Line type="monotone" dataKey="seconds" stroke="#3B82F6" strokeWidth={2} dot={false} isAnimationActive={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </section>
      ...
      <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-surface">
        ...
        {perBank.map((b) => {
          ...
          return (
            <div key={b.id} className="grid grid-cols-[1fr_90px_110px] items-center gap-4 border-b border-border px-6 py-3 last:border-b-0">
              <div className="flex items-center gap-3 min-w-0">
                <span className="h-2.5 w-2.5 flex-shrink-0 rounded-full" style={{ backgroundColor: b.hex }} />
                <span className="truncate text-sm font-semibold text-foreground">{b.name}</span>
              </div>
              <span className={`text-right font-mono text-sm font-bold tabular-nums ${c.text}`}>{b.pct}%</span>
              <span className="text-right font-mono text-sm tabular-nums text-foreground/80">{b.answered.toLocaleString()}</span>
            </div>
          );
        })}
      </section>
