      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Managed exam categories that question banks can be mapped to. {examTypes.length} total ·{" "}
            {activeCount} active · {totalBanks} banks mapped.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border bg-surface p-12 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Try a different search, or add a new exam type.
          </p>
        </div>
      ) : (
                  ...
                  <div className="mt-4 flex items-center gap-2 border-t border-border pt-4">
                    ...
                    <button
                      onClick={() => {
                        if (et.bankCount > 0) {
                          toast.error(
                            `Can't delete — ${et.bankCount} banks still use "${et.name}"`,
                          );
                          return;
                        }
                        setDeleting(et);
      ...
      >
        <header className="border-b border-border px-5 py-4">
          <h3 className="text-base font-bold text-foreground">
            {initial ? "Edit exam type" : "New exam type"}
          </h3>
        </header>
        <div className="space-y-4 p-5">
          ...
          <div className="flex items-center justify-between rounded-lg border border-border bg-surface-alt/40 px-4 py-3 text-sm font-medium text-foreground">
            ...
            <ToggleSwitch
              ...
              ariaLabel="Active — available when creating banks"
            />
          </div>
        </div>
        <footer className="flex justify-end gap-2 border-t border-border px-5 py-3">
          ...
          <button
            disabled={!name.trim()}
            onClick={() =>
              onSave({ name: name.trim(), description: description.trim(), color, icon, active })
            }
