...
export const Route = createFileRoute("/admin/questions/create")({
  ...
  head: () => ({ meta: [{ title: "Admin · Create Question — Medinovaqbank" }, { name: "robots", content: "noindex" }] }),
...
function CreateQuestion() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/admin/questions" className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          {lockBank ? questionBanks.find((b) => b.id === bankId)?.name : "Choose a bank and author a new question."}
        </p>
      </div>
